					...
					<dd class="dd-padding">
						<div class="merchant">
						  ...
						  <c:if test="${isFollow == 'false'}">
						     <div class="text-icon"   onclick="submit(${cacheUser.mchUser.mchId},'add')">
						       <font size="5" color="blue">❤关注 </font>
						    </div>
						  </c:if>	
						  <c:if test="${isFollow == 'true' }">
						   <div class="text-icon"   onclick="submit(${cacheUser.mchUser.mchId},'delete')">
...
function showMessage(message,type,time) {
 ...
 var submit = function(id,opt){
	...
	$.post("<%=request.getContextPath()%>/wechatuser/followmch",{mchid:id,type:opt},function(data){
